<?php
	...
	if($row){	header("Location: http://localhost/exam/choce.php");}
	else{echo '登录失败！';}
	}
	...
	$email=$_GET['email'];
	$answer=$_GET['answer'];
	$sql="select passwd from tb_user where name='$name' and email='$email' and answer='$answer'";
	$rs=mysql_query($sql);
	$row=mysql_fetch_array($rs);
	if($row){
	header("Location: http://localhost/exam/showpasswd.php?passwd=$row[0]");
	exit;
	}
	}
	...
	{
	$name=$_GET['name'];
	$sql="select question from tb_user where name='$name'";
	$rs=mysql_query($sql);
	$row=mysql_fetch_array($rs);
	echo $row[0];
	}
	...
	$row=mysql_fetch_array($rs);
	if($row){
	header("Location: http://localhost/exam/src/bg_find.php");
	exit;
	}
	else{
	}
	...
	$result = array();
	$result["total"] =1;
	$sql="select * from tb_user where name='$name' limit $offset,$rows";
	$rs = mysql_query($sql);
	$items = array();
	while($row = mysql_fetch_object($rs)){
		array_push($items, $row);
	...
	$sql = "insert into tb_user(name,passwd,age,sex,phone,email,question,answer) values('$name','$passwd','$age','$sex','$phone','$email','$question','$answer')";
	mysql_query("set names 'utf8'");//写库 
	$result = @mysql_query($sql);
	if ($result){
		echo json_encode(array('success'=>true));
	...
	$sql = "insert into tb_test  values('$testid','$subject','$question','$answera','$answerb','$answerc','$answerd','$rightanswer','$jiexi','$score')";
	$result = @mysql_query($sql);
	if ($result){
		echo json_encode(array('success'=>true));
	} else {
	...
	$sql = "update tb_score set name='$name',subject='$subject',score='$score',dadetime='$dadetime' where id='$id'";
	$result = @mysql_query($sql);
	if ($result){
		echo json_encode(array('success'=>true));
	} else {
